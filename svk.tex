...
Definujeme \textbf{krok výpočtu} ako reláciu $\vdash$ na konfiguráciách nasledovne:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{theorem}
\begin{proof}
Nech $\alpha\in\nle$. Zostrojíme Turingov stroj $T$, ktorý bude akceptovať $L(\alpha)$ a na pracovných páskach nezapíše viac ako $O(\log^2 n)$ políčok.
...
$T$ bude skonštruovaný ako Turingov stroj vo vete \ref{lel_dspace}. Jediné, čo je treba, je dodefinovať jeho správanie, keď narazí na negatívny lookaround. Predpokladajme, že $\alpha$ obsahuje práve jeden negatívny lookaround. Vnútri tohto lookaroundu je nejaký regex $\beta$ taký, že $\beta\in\le$. Ten vieme podľa vety simulovať deterministickým Turignovým strojom a spotrebuje pamäť $O(\log^2|\beta|)$.
\end{proof}
...
Myšlienka je podobná dôkazu Savitchovej vety\cite{Savitch}. Turingov stroj T bude testovať, či sa dá dostať z konfigurácie $C_1$ do konficurácie $C_2$ na $i$ krokov:
...
bool $TESTUJ(C_1,C_2,i)$
	...
	return false
\end{lstlisting}
Konfigurácie budú zodpovedať formálnemu modelu a ako v predošlom dôkaze budú na páske zaznamenané ako niekoľko adries -- aktuálna pozícia a pracovná pozícia v regexe, aktuálna a pracovná pozícia v slove, začiatok a koniec podslova pre $k$-te indexovateľné zátvorky pre $\forall k$ $(z)$, začiatok každého lookaheadu $(l_a)$ a lookbehindu $(l_b)$ -- spolu $2\log r+(2z+l_a+l_b)\cdot \log w \leq O(r\log w)$ pamäte.
